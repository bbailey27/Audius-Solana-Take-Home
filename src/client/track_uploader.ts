...
class TrackInstruction {
  ...
  constructor(
    fields:
      | { variant: TrackInstructionVariant; id: string }
      | undefined = undefined
  ) {
    // super();
    if (fields) {
...
export async function establishConnection(): Promise<void> {
  ...
  console.log(
    "ESTABLISH CONNECTION: Connection to cluster established:",
    rpcUrl,
    version
  );
}
...
function createTrackInstructionData(
  variant: TrackInstructionVariant,
  id: string
): Buffer {
  ...
  const dataSerBuf = Buffer.from(
    borsh.serialize(TrackInstructionSchema, instructionData)
  );
  console.log("buf", dataSerBuf);
  console.log(
    "deserialized",
    borsh.deserialize(TrackInstructionSchema, TrackInstruction, dataSerBuf)
  );
  return dataSerBuf;
}
...
export async function getTrack(trackId: string): Promise<void> {
  ...
  console.log("Fetched track with ID", trackId, "and it has CID", track.cid);
}
